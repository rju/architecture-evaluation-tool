...
import de.cau.cs.se.evaluation.architecture.transformation.java.old.GlobalJavaScope;
import de.cau.cs.se.evaluation.architecture.transformation.java.old.JavaLocalScope;
import de.cau.cs.se.evaluation.architecture.transformation.java.old.JavaPackageScope;
import de.cau.cs.se.evaluation.architecture.transformation.java.old.JavaTypeHelper;
import de.cau.cs.se.evaluation.architecture.transformation.java.old.ResolveStatementForClassAccess;
...
public class TransformationJavaClassesToHypergraph implements ITransformation {
  @Extension
   ...
   */
  public TransformationJavaClassesToHypergraph(final IScope scope, final List<IType> classList) {
   ...
   */
  public TransformationJavaClassesToHypergraph(final GlobalJavaScope scope, final List<IType> classList, final IProgressMonitor monitor) {
    this.globalScope = scope;
    this.classList = classList;
      ...
      if (_isClass) {
        ...
        final Consumer<IType> _function_1 = new Consumer<IType>() {
          public void accept(final IType destinationType) {
            ...
            _edges.add(edge);
            EList<Node> _nodes = system.getNodes();
            Node _findNode = TransformationJavaClassesToHypergraph.this.findNode(_nodes, sourceType);
            EList<Edge> _edges_1 = _findNode.getEdges();
            _edges_1.add(edge);
            EList<Node> _nodes_1 = system.getNodes();
            Node _findMatchingNode = TransformationJavaClassesToHypergraph.this.findMatchingNode(_nodes_1, destinationType);
  ...
  private Node findMatchingNode(final List<Node> nodes, final IType type) {
    try {
      ...
      if (_equals) {
        ...
        final Function1<IType, Boolean> _function = new Function1<IType, Boolean>() {
          public Boolean apply(final IType it) {
            Node _findNode = TransformationJavaClassesToHypergraph.this.findNode(nodes, it);
